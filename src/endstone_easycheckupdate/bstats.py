...
# 创建bStats专用的logger
bstats_logger = logging.getLogger(f"EasyUpdateBStats")
...
class BStatsConfig:
    ...
    def __init__(self, config_file: Path, translate=None):
        ...
        self.translate = translate or (lambda key, *args: key)
        self.enabled = True
    ...
    def _load_config(self):
        ...
        try:
            ...
            if self.config_file.exists():
                ...
                status_text = self.translate("general.enabled") if self.enabled else self.translate("general.disabled")
                bstats_logger.info(self.translate("bstats.status", status_text))
            else:
...
class BStats:
    ...
    def __init__(self, plugin, service_id: int, translate=None):
        """
        初始化 bStats
        ...
        Args:
            ...
            translate: 可选的翻译回调函数，签名为 translate(key, *args) -> str
        """
        self.plugin = plugin
        ...
        try:
            ...
            self.config = BStatsConfig(bstats_folder / "config.json", translate=self.translate)
        except Exception as e:
    ...
    def _submit_data(self):
        ...
        if not self.config.enabled:
            bstats_logger.info(self.translate("bstats.disabled_skip"))
            return
        ...
        try:
            ...
            bstats_logger.info(self.translate("bstats.submitting"))
            ...
            if response.status_code == 200:
                bstats_logger.info(self.translate("bstats.submit_success"))
                if self.config.log_sent_data_enabled:
                    bstats_logger.info(f"响应内容: {response.text if response.text else '(空)'}")
            else:
                bstats_logger.warning(self.translate("bstats.submit_failed", str(response.status_code)))
                if self.config.log_sent_data_enabled:
                    bstats_logger.warning(f"返回结果: {response.text if response.text else '(空)'}")

        except Exception as e:
            bstats_logger.error(self.translate("bstats.network_error", str(e)))
    ...
    def _submit_loop(self):
        ...
        while self._running:
            try:
                ...
                bstats_logger.error(self.translate("bstats.submit_error", str(e)))
                if self.config.log_errors_enabled:
                    import traceback
                    bstats_logger.error(f"异常堆栈: {traceback.format_exc()}")
    ...
    def start(self):
        ...
        # 输出启动日志
        bstats_logger.info(self.translate("bstats.module_started", self.plugin_name))
        bstats_logger.info(self.translate("bstats.first_submit_info"))
        if self.config.log_sent_data_enabled:
            bstats_logger.info(self.translate("bstats.plugin_info", str(self.service_id), self.plugin_version))
            status_text = self.translate("general.enabled") if self.config.enabled else self.translate("general.disabled")
            bstats_logger.info(self.translate("bstats.status", status_text))
            debug_text = self.translate("general.enabled") if self.config.log_sent_data_enabled else self.translate("general.disabled")
            bstats_logger.info(self.translate("bstats.debug_mode", debug_text))
    ...
    def shutdown(self):
        ...
        if self.config.log_sent_data_enabled:
            bstats_logger.info(self.translate("bstats.module_stopped", self.plugin_name))
